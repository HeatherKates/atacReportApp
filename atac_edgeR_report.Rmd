...
load(paste0(report_params$nfcore_output_dir, "/bowtie2/merged_library/macs2/broad_peak/consensus/deseq2/consensus_peaks.mLb.clN.dds.RData"))
...
peaks_anno <- read.delim(paste0(report_params$nfcore_output_dir,"bowtie2/merged_library/macs2/broad_peak/consensus/consensus_peaks.mLb.clN.annotatePeaks.txt"))
...
mmu_chrom_sizes="/R/mm10.chrom.sizes"
hsa_chrom_sizes="/R/hg38.chrom.sizes"
...
# Set variables 
  ...
  Sys.setenv(pipeline_output = report_params$nfcore_output_dir, 
...
trackhub_base_url <- "https://data.rc.ufl.edu/pub/cancercenter-dept"
...
for (contrast in names(results_list)) {
  ...
  base_path <- paste0(trackhub_base_url, "/", report_params$seqID, "/", contrast_clean)

  hub_links[[contrast]] <- paste0(
    "https://genome.ucsc.edu/cgi-bin/hgTracks?hubUrl=", base_path, "/hub.txt"
  )
  igv_links[[contrast]] <- paste0(base_path, "/igv/", contrast_clean, "_igv_session.zip")
}

hub_links_md <- sapply(hub_links, function(url) {
  sprintf("[View Tracks in UCSC Browser](%s)", url)
})
